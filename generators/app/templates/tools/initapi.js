...
_.each(models, (Model, name) => {
  ...
  primaryKeys.forEach(key => {
    ...
    let regexp;
    if (['INTEGER', 'BIGINT'].indexOf(paramType) > -1) {
      // number types
  ...
  let routes = [
    ...
    { method: 'delete', path: `/${name}s/${params_str}`, func: 'destroy' }
  ];
  ...
  routes
    ...
    .forEach(item => {
      routesContent += `${routerVar}.${item.method}('${item.path.replace(
        '\\',
        '\\\\'
      )}', ${controllerVar}.${name}.${item.func});\n`;
    });
